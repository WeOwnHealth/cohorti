# @passport/verifier-api

The one endpoint a verifier ever calls: `POST /proof-requests`. Orchestrates the policy gate (`@passport/disclosure-guard`) and the Midnight prover, and returns **pass/fail + recency only** — no raw data, no fallback on refusal.
...
The orchestration logic is real; two pieces are intentionally stubbed and marked with `TODO(passport)`:
...
- **`src/routes.ts`** — the credential's real binding tier needs to come from `@passport/issuance`, not be hardcoded.

## Run

```bash
pnpm --filter @passport/verifier-api dev
```

Requires `@passport/disclosure-guard` running (`DISCLOSURE_GUARD_URL`, default `http://localhost:4007`).
